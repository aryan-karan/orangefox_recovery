...
void TWFunc::Run_Before_Reboot(void)
{
    ...
    TWFunc::write_to_file(Fox_Logs_Dir + "/releaseinfo.json", "{\"json_ver\": \"1\", \"codename\":\""+Fox_Current_Device+
...
void TWFunc::Welcome_Message(void)
{
   if (Fox_Has_Welcomed > 0)
    ...
    gui_print("[Release]   : %s\n", FOX_BUILD);
    gui_print("[Codebase]  : %s, %s\n", SDK Version: Fox_Property_Get("ro.build.version.sdk").c_str(), FOX_CURRENT_DEV_STR);
